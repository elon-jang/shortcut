    ...
    <div className="max-w-lg md:max-w-xl mx-auto flex flex-col items-center animate-in fade-in duration-300">
      ...
      <div className="w-full flex justify-between items-center mb-3 px-1">
        <div className="flex items-center gap-2">
          <button
            onClick={onBack}
            className="flex items-center gap-1 text-slate-500 dark:text-slate-600 hover:text-indigo-500 transition-colors mr-1"
            title="모드 선택으로 (Esc)"
          >
            <ArrowLeft size={14} />
          </button>
          ...
          <div className="flex items-center gap-1 text-[11px] md:text-xs font-bold text-indigo-400 bg-slate-100 dark:bg-slate-800/60 px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700/50">
      ...
      {/* Progress bar */}
      <div className="w-full h-1 bg-slate-200 dark:bg-slate-800/80 rounded-full mb-5 overflow-hidden">
        <div
      ...
      <div className={`w-full bg-white dark:bg-slate-800/90 border rounded-2xl px-6 py-5 md:px-8 md:py-6 text-center shadow-lg relative transition-all duration-300 ${
        ...
        <h2 className="text-lg md:text-xl font-black mb-5 md:mb-6 leading-snug tracking-tight text-slate-900 dark:text-white break-keep">
          {question.action}
        </h2>
        ...
        {(feedback === 'correct' || feedback === 'wrong') && question.tip && (
          <div className="mb-4 py-2 px-4 bg-amber-500/8 border border-amber-500/20 rounded-xl text-left">
            ...
            <p className="text-amber-600 dark:text-amber-200 text-xs md:text-sm">{question.tip}</p>
          </div>
        )}
