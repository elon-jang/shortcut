...
export const ResultScreen = ({ activeCategory, score, timer, missCount = 0, totalQuestions = 0, onRestart, onModeSelect, onMenu, onViewList, sessionResults = [] }) => {
  ...
  return (
    <div className="max-w-md md:max-w-lg mx-auto bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 md:p-8 text-center animate-in zoom-in-95 duration-500 shadow-xl">
      <div className="inline-flex p-4 bg-yellow-500/10 rounded-2xl mb-4 border border-yellow-500/15 text-yellow-500">
        <Trophy size={36} />
      </div>
      <h2 className="text-2xl md:text-3xl font-black mb-1 tracking-tight text-slate-900 dark:text-white">수련 완료</h2>
      ...
      <div className="grid grid-cols-3 gap-2.5 md:gap-3 mb-5">
        <div className="bg-slate-100 dark:bg-slate-900/60 p-3 md:p-4 rounded-xl border border-slate-200 dark:border-slate-700/50">
          ...
          <div className="max-h-48 overflow-y-auto space-y-1 pr-1 scrollbar-thin">
            {sessionResults.map((r, i) => (
              <div key={i} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs md:text-sm ${
                ...
                <span className="truncate flex-1 text-slate-700 dark:text-slate-300">{r.action}</span>
                <span className="font-mono text-slate-600 dark:text-slate-400 shrink-0 text-[11px]">{r.keys}</span>
                {r.newBox !== r.prevBox && (
      ...
      <div className="mb-4 px-3 py-2 bg-indigo-500/8 border border-indigo-500/15 rounded-lg">
        <p className="text-[10px] md:text-xs text-indigo-500 dark:text-indigo-300/70">
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-2.5 md:py-3 bg-slate-100 dark:bg-slate-700/80 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-xl font-bold text-sm md:text-base border border-slate-200 dark:border-slate-600/50 transition-all flex items-center justify-center gap-2 active:scale-[0.97] text-slate-700 dark:text-white"
          >
          ...
          <button
            onClick={onMenu}
            className="flex-1 py-2.5 md:py-3 bg-slate-50 dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl font-bold text-sm md:text-base border border-slate-200 dark:border-slate-700/50 transition-all flex items-center justify-center gap-2 active:scale-[0.97] text-slate-500 dark:text-slate-400"
          >
            <Home size={14} /> 메뉴
          </button>
        </div>
        <button
          onClick={onViewList}
          className="w-full py-2 flex items-center justify-center gap-1.5 text-slate-500 dark:text-slate-600 hover:text-slate-700 dark:hover:text-slate-400 transition-colors text-xs font-medium"
        >
