    ...
    <div className="perspective-2000 relative z-50 min-h-[140px] md:min-h-[160px]">
      <div className={`relative w-full h-full min-h-[140px] md:min-h-[160px] transition-all duration-500 preserve-3d ${flipped ? 'rotate-y-180' : ''}`}>
        ...
        >
          ...
          <span className="text-[11px] text-slate-400 dark:text-slate-600 font-mono">Space</span>
        </div>

        {/* Back - answer */}
        <div className={`absolute inset-0 backface-hidden rotate-y-180 bg-white dark:bg-slate-900/90 rounded-xl border-2 border-indigo-400 dark:border-indigo-500/50 flex flex-col items-center justify-center gap-4 shadow-[0_0_30px_rgba(99,102,241,0.15)] z-30 ${!flipped ? 'pointer-events-none invisible' : 'pointer-events-auto visible'}`}>
          <span className="text-2xl md:text-3xl font-mono font-black text-indigo-600 dark:text-indigo-400 tracking-tight">
            {answer}
          </span>
          {tip && (
          ...
          <div className="flex gap-3">
            <button
              ...
              className="px-5 py-2 md:px-6 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl text-sm md:text-base font-bold border border-slate-200 dark:border-slate-700 transition-all text-slate-700 dark:text-slate-300 active:scale-95 flex items-center gap-2"
            >
